...
if(isset($_POST["from_date"], $_POST["to_date"], $_POST["id"]))
{
    $emp_id=$_POST['id'];
    $output= '';
    $res= mysqli_query($db,"SELECT * FROM history_bonus WHERE emp_id='$emp_id' AND `date` BETWEEN '".$_POST["from_date"]."' AND '".$_POST["to_date"]."' ORDER BY `serial` DESC");
    ...
    if($res -> num_rows > 0)
    {
        while ($row=mysqli_fetch_array($res)) {
            ...
            $output .= "<td><a href='edit_remark_bon.php?id=".$row['serial']."' class='glyphicon glyphicon-pencil'></a></td>";
            $output .= "</tr>";
        }
    }
    else
